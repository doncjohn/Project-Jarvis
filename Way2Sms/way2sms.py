...
def valid_numbers(numbers):
    for number in numbers:
        if len(number) <> 10:
            print
            'Number', number, 'isn\'t a valid Indian mobile number.'
            return False
    return True
...
def valid_message(message):
    if len(message) > 140:
        print
        'Your message exceeds the limit of 140 characters.'
        return False
    return True
...
def send_sms(numbers, message):
    ...
    try:
        ...
        print
        'Authentication failed, check your login details again.'
        return False
    else:
        print
        'Were hooked on to the Way2SMS server!'
...
def print_usage():
    print
    'usage: python', sys.argv[0], '--numbers=no1:no2:no3 --message="Your message here"'
